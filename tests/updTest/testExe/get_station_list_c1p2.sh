...
#   echo "$rec_type"
       ...
       # find receiver type in 'c1p2.txt'
    found=`awk -v rec="$rec_type" 'BEGIN{ i=0; } { if(rec==substr($0,1)) { i=i+1; } } END  {print i;}' c1p2.txt`
    ...
    if [[ $found -gt 0 ]];then
       ...
       if [[ "$IS" == "Y" ]]; then
          ...
          echo $Lon $Lat 4 0 4 CB $STATION >> station.c1p2.llh
          echo $STATION >> station.c1p2.list
