...
awk '/\+SITE\/ID/,/\-SITE\/ID/' ./ephemeris/igs11P1656.ssc > site.txt

   # Get station name list
ls ./rinex/*2810.11o > rinex.list
...
#   echo "$rec_type"
       ...
       # find receiver type in 'c1only.txt'
    found=`awk -v rec="$rec_type" 'BEGIN{ i=0; } 
    { if(rec==substr($0,1)) { i=i+1; } } END  {print i;}' c1only.txt`
    ...
    if [[ $found -gt 0 ]];then
       ...
       if [[ "$IS" == "Y" ]]; then
          ...
          echo $Lon $Lat 4 0 4 CB $STATION >> station.c1only.llh
          echo $STATION >> station.c1only.list
       else
          echo "$STATION is not in igs11P1656"
       fi
    fi
