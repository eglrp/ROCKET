...
namespace gpstk
{
   ...
   bool Rinex3EphemerisStore2::addEphemeris(const Rinex3NavData& inRdata)
   {
...
#pragma unused(ts)
   ...
   int Rinex3EphemerisStore2::loadFile(const string& filename, bool dump, ostream& s)
   {
      try {
         int nread(0);
   ...
   } // end Rinex3EphemerisStore2::loadFile
   ...
   string Rinex3EphemerisStore2::dumpTimeSystemCorrection(
   ...
   CommonTime Rinex3EphemerisStore2::correctTimeSystem(const CommonTime ttag,
   ...
   Xvt Rinex3EphemerisStore2::getXvt(const SatID& sat, const CommonTime& inttag) const
   {
      try {
         Xvt xvt;
         CommonTime ttag;
   ...
   void Rinex3EphemerisStore2::dump(ostream& os, short detail) const
   {
      ...
      if(gpsStore.size()) {
         os << "Dump of GPS ephemeris store:\n";
         gpsStore.dump(os, detail);
      }

      if(galStore.size()) {
         os << "Dump of GAL ephemeris store:\n";
         galStore.dump(os, detail);
      }

      if(bdsStore.size()) {
         os << "Dump of BDS ephemeris store:\n";
         bdsStore.dump(os, detail);
      }
   ...
   CommonTime Rinex3EphemerisStore2::getInitialTime(void) const
   {
      try {
   ...
   // @throw InvalidRequest if the object has no data.
   CommonTime Rinex3EphemerisStore2::getFinalTime(void) const
   {
      try {
   ...
   // @return the earliest time in the table
   // @throw InvalidRequest if the object has no data.
   CommonTime Rinex3EphemerisStore2::getInitialTime(const SatID& sat) const
   {
      try {
         ...
         switch(sat.system) {
            case SatID::systemGPS:
                retTime = gpsStore.getInitialTime(sat);
            case SatID::systemGalileo:
                retTime = galStore.getInitialTime(sat);
            case SatID::systemBDS:
                retTime = bdsStore.getInitialTime(sat);
            default:
                break;
         }
   ...
   // @return the latest time in the table
   // @throw InvalidRequest if the object has no data.
   CommonTime Rinex3EphemerisStore2::getFinalTime(const SatID& sat) const
   {
      try {
         if(sat.system == SatID::systemMixed)
            return getFinalTime();
         ...
         switch(sat.system) {
            case SatID::systemGPS:
                retTime = gpsStore.getFinalTime(sat);
            case SatID::systemGalileo:
                retTime = galStore.getFinalTime(sat);
            case SatID::systemBDS:
                retTime = bdsStore.getFinalTime(sat);
            default:
                break;
         }
   ...
   int Rinex3EphemerisStore2::addToList(list<Rinex3NavData>& theList, SatID sysSat)
      const
   {
      int n(0);
      ...
      if(keepBDS) {
          ...
          list<BDSEphemeris> bdsList;
          n += bdsStore.addToList(bdsList);

          list<BDSEphemeris>::const_iterator it;

          for(it=bdsList.begin(); it!=bdsList.end(); ++it)
              theList.push_back(Rinex3NavData(*it));
      }
