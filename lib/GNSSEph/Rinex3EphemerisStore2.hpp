...
#ifndef GPSTK_RINEX3EPHEMERISSTORE2_HPP
#define GPSTK_RINEX3EPHEMERISSTORE2_HPP
...
namespace gpstk
{
   ...
   class Rinex3EphemerisStore2 : public XvtStore<SatID>
   {
   // member data:
   private:
   ...
   // member functions:
      ...
      {
         bool flag_gps = gpsStore.getOnlyHealthyFlag();
         bool flag_gal = galStore.getOnlyHealthyFlag();
         bool flag_bds = bdsStore.getOnlyHealthyFlag();
      }

      /// destructor
      virtual ~Rinex3EphemerisStore2()
      { }


      GPSEphemerisStore getGPSEphemerisStore() const
      { return gpsStore; }

      GalEphemerisStore getGalEphemerisStore() const
      { return galStore; }

      BDSEphemerisStore getBDSEphemerisStore() const
      { return bdsStore; }
   ...
   // XvtStore interface:
                        ...
                        const CommonTime& tmax = CommonTime::END_OF_TIME)
      {
         ...
         if(bdsStore.size()) bdsStore.edit(tmin, tmax);
      }
      ...
      virtual bool isPresent(const SatID& sat) const
      {
         switch(sat.system) {
            case SatID::systemGPS:
                return gpsStore.isPresent(sat);
            case SatID::systemGalileo:
                return galStore.isPresent(sat);
            case SatID::systemBDS:
                return bdsStore.isPresent(sat);
            default:
                return false;
         }
         return false;
   ...
   // end of XvtStore interface
      ...
      int size(const SatID sat) const
      {
         ...
         if(sys == SatID::systemMixed)
             n = gpsStore.size(sat) + galStore.size(sat) + bdsStore.size(sat);

         if(sys == SatID::systemGPS)
             n = gpsStore.size(sat);
         if(sys == SatID::systemGalileo)
             n = galStore.size(sat);
         if(sys == SatID::systemBDS)
             n = bdsStore.size(sat);
      ...
      bool getOnlyHealthyFlag(void) const
      {
         return gpsStore.getOnlyHealthyFlag()
             && galStore.getOnlyHealthyFlag()
             && bdsStore.getOnlyHealthyFlag();
      }
      ...
      void setOnlyHealthyFlag(bool flag)
      {
         ...
         bdsStore.setOnlyHealthyFlag(flag);
      }
   ...
   }; // end class Rinex3EphemerisStore2

}  // namespace

#endif // GPSTK_RINEX3EPHEMERISSTORE2_HPP
