...
// http://www.ngs.noaa.gov/IGSWorkshop2008/docs/Schaer_DCB_IGSWS2008.ppt
               ...
               if(useDCB)
               {
                  ...
                  stv->second[TypeID::instP2] = (kappa1 * dcb);
						...
						if( CorrectRecInstP2 )
						{
								// This means that we should corrrect recInsP2
								// The strategy adopted here is to let the 
								// TypeID::instP2 absorb this quantity
								// But we shoud be careful with the sign
							double recInsP2(0.0);

							try
							{
									// At present, set all the source type as GPS
								targrtSource.setSourceType( SourceID::GPS );
								recInsP2 = pDefaultMaps -> findStationDCB( targrtSource, 
																				       time );

								// Debug code vvv
								
								cout << "targrtSource: " << targrtSource 
										<< " dcb: " << recInsP2 << endl;

								// Debug code ^^^ 

									// Change the unit
								recInsP2 = recInsP2 * C_MPS * 1e-9;

							}
							catch( Exception& e )
							{
								e.addText(" Problem occurs in finding station dcb ");								
								GPSTK_THROW( e );
							}

							stv->second[TypeID::instP2] -= recInsP2;

						}  // End of 'if( CorrectRecInstP2 )'
