#pragma ident "$Id: SolverGenWLMGEX.cpp 2346 2012-05-23 15:55:58Z shjzhang $"

/**
 * @file SolverGenWLMGEX.cpp
...
//              will be biased.
//              Thus, in the 'SolverGenWLMGEX' class, only the independent 
...
namespace gpstk
{
   ...
   int SolverGenWLMGEX::classIndex = 9510000;


      // Returns an index identifying this object.
   int SolverGenWLMGEX::getIndex() const
   { return index; }


      // Returns a string identifying this object.
   std::string SolverGenWLMGEX::getClassName() const
   { return "SolverGenWLMGEX"; }
       ...
       */
   gnssSatTypeValue& SolverGenWLMGEX::Process(gnssSatTypeValue& gData)
      throw(ProcessingException)
   {
   ...
   }  // End of method 'SolverGenWLMGEX::Process()'
       ...
       * @param gData     Data object holding the data.
       */
   gnssRinex& SolverGenWLMGEX::Process(gnssRinex& gData)
      throw(ProcessingException)
   {
       ...
       */
   gnssDataMap& SolverGenWLMGEX::Process(gnssDataMap& gdsMap)
      throw(ProcessingException)
   {
      ...
      try
      {
         ...
         Predict( gdsMap );
			...
			// Test code vvv
			//cout << " cpu time for Predict function in SolverGenWLMGEX.cpp: " 
			//		<< Counter::end() << endl;
			// Test code ^^^
         ...
         Correct( gdsMap );
			...
			// Test code vvv
			//cout << " cpu time for Correct function in SolverGenWLMGEX.cpp: " 
			//		<< Counter::end() << endl;
			// Test code ^^^
            // return
         return gdsMap;
       ...
       * @param gData    Data object holding the data.
       */
   gnssDataMap& SolverGenWLMGEX::Predict( gnssDataMap& gdsMap )
      throw(ProcessingException)
   {
...
//			// Debug code vvv
//			cout << "sats used in equSystem: " << endl;
//			SatIDSet satSet( equSystem.getCurrentSats() );
...
////////////////////////// //////////////////////////
   ...
   }  // End of method 'SolverGenWLMGEX::preCompute()'
      ...
      //
   gnssDataMap& SolverGenWLMGEX::Correct( gnssDataMap& gdsMap )
      throw(ProcessingException)
   {
      ...
      try
      {
         ...
         MeasUpdate( gdsMap );
				...
				// Added by Lei Zhao, 2016/10/05 vvv (^_^)
				// Time Counter setting
			//std::cout << "cpu time for MeasUpdate function in SolverGenWLMGEX.cpp: "
         ...
         AmbiguityFixing( gdsMap );
			...
			// Test code vvv
			//std::cout << "cpu time for AmbiguityFixing function in SolverGenWLMGEX.cpp: "
			//		<< Counter::end() << std::endl;
			// Test code ^^^
...
//			Counter::begin();
         ...
         postCompute(gdsMap);
			...
			// Test code vvv
//			std::cout << "cpu time for postCompute function in SolverGenWLMGEX.cpp: "
//					<< Counter::end() << std::endl;
			// Test code ^^^ 
      }
      catch(Exception& u)
   ...
   }  // End of method 'SolverGenWLMGEX::Compute()'
       ...
       * @param gData    Data object holding the data.
       */
   gnssDataMap& SolverGenWLMGEX::MeasUpdate( gnssDataMap& gdsMap )
      throw(ProcessingException)
   {
      try
      {
                 ...
                 ++itVar )
            {
               ...
               if( (*itVar).getType() == TypeID::BWL )
               {
                  ...
                  //xhatminus(i) = std::floor( BWL/(-0.861918400) + 0.5 );
						...
						double waveLen(1.0);
						SatID sat( (*itVar).getSatellite() );
						
						if( sat.system == SatID::systemGPS )
						{
							waveLen = -1*WL_WAVELENGTH_GPS;
						}

						if( sat.system == SatID::systemGalileo )
						{
							waveLen = -1*WL_WAVELENGTH_GAL_L1L5;
						}

						xhatminus(i) = std::floor( BWL/(waveLen) + 0.5 );
...
//				// Remove bad sat and station pair
//				// At wanted epoch 
//			SatID GPS9( 9, SatID::systemGPS );
//			SourceID nya2( SourceID::Mixed, "NYA2" );
//			CivilTime time( (gdsMap.begin())->first );
////			if( time.hour == 18 && time.minute == 10 )
////			{
//			VariableDataMap ambMapTemp(ambMap);
//				
//			for( VariableDataMap::iterator ite = ambMapTemp.begin(); 
//				  ite != ambMapTemp.end(); 
//				  ++ite )
//			{
//				Variable amb( ite->first );
//					
//					// Bad pair: sat9,NYA2, at epoch 18:10
//				if( time.hour == 18 && time.minute == 10 && 
//					 amb.getSatellite() == GPS9 &&
//					 amb.getSource() == nya2 )
//				{
//					cout << "find the bad pair" << endl;
//						// Remove the pair from ambMap
//					ambMap.erase( amb );
//					ambVarMap.erase( amb );
//
//						// And currentUnknowns 
//					currentUnknowns.erase( amb );
//				}
//
//			}

//			}  // End of 'if( time.hour == 18 && ...'
			
//			cout << "All the BWL amb: " << endl;
//			for( VariableDataMap::iterator ite = ambMap.begin(); 
//				  ite != ambMap.end(); 
//				  ++ite )
//			{
//				Variable amb( ite->first );
//				double value( ite->second );
//				cout << amb.getSatellite() << " " 
//						<< amb.getSource() << ": " << value << endl; 
//			}  // End of ' for( VariableDataMap::iterator ite ... '
			// ^^^ Debug code ^^^ 
...
//			// Debug code vvv
//			cout << "All the independent amb: " << endl;
//			for( VariableDataMap::iterator ite = ambFixedMap.begin(); 
//				  ite != ambFixedMap.end(); 
//				  ++ite )
//			{
//				Variable amb( ite->first );
...
//				// Debug code vvv
//					// Output the statistics of postfitResiduals at present epoch 
//			Stats<double> statTest;
//         int rowTest(0);
//         for( std::list<Equation>::const_iterator itEq = equList.begin();
//              itEq != equList.end();
//              ++itEq )
//			{
//					// Get equation type 
//				Variable equType( (itEq -> header).indTerm );
////
//				cout << equType.getType() << "( ";
//
//
//            for( VarCoeffMap::const_iterator vcmIter = (*itEq).body.begin();
//                 vcmIter != (*itEq).body.end();
...
////				if( equType.getSatIndexed() )
////				{
////					cout << equType.getSatellite() << " ";
////				}
////				if( equType.getSourceIndexed() )
////				{
////					cout << equType.getSource() << " ";
////				}
////				
////					// Output its postfit residuals
//				cout << ") postfitRes: " << postfitResiduals( rowTest ) << endl;
//
//					// Add to stats obj
////				statTest.Add( postfitResiduals( rowTest ) );
//
//				rowTest++;
//			}  // End of ' for( std::list<Equation>::const_iterator itEq ... '
//				
////			cout << "postfitResidual ave: " << statTest.Average() << " "
////				  << "std: " << statTest.Variance() 
////				  << endl;
//				// Debug code ^^^ 
   ...
   gnssDataMap& SolverGenWLMGEX::AmbiguityFixing( gnssDataMap& gData )
      throw(ProcessingException)
   {
...
//					<< Counter::end() << endl;
   ...
   }  // End of method 'SolverGenWLMGEX::AmbiguityFixing()'
       ...
       */
   void SolverGenWLMGEX::AmbiguityUpdate( Vector<double>& state,
      ...
      throw(ProcessingException)
   {
   ...
   }  // End of method 'SolverGenWLMGEX::AmbiguityUpdate()'
      ...
      // @param gData    Data object holding the data.
      //
   gnssDataMap& SolverGenWLMGEX::postCompute( gnssDataMap& gdsMap )
      throw(ProcessingException)
   {
...
//         }  // End of for( VariableSet::const_iterator itVar1 = currentUnknowns...'
                  ...
                  else
                  {
                     ...
                     if(tvmIter!= tData.end())
                     {
                        ...
                        //(*stvmIter).second[TypeID::BLC] = BLC;
								...
								{
									//BLC = tData( TypeID::BLC ) + 0.377482511 * NWL;
									BLC = tData( TypeID::BLC ) + 
											factor1_GPS * WL_WAVELENGTH_GPS * NWL;
								}

								if( sat.system == SatID::systemGalileo )
								{
									BLC = tData( TypeID::BLC ) +
											factor1_GAL * WL_WAVELENGTH_GAL_L1L5 * NWL;
								}
   ...
   }  // End of method 'SolverGenWLMGEX::postMeasUpdate()'
       ...
       */
   double SolverGenWLMGEX::getSolution( const TypeID& type,
   ...
   }  // End of method 'SolverGenWLMGEX::getSolution()'
      ...
      throw(InvalidRequest)
   {
       ...
       * of this type, only the first one will be returned.
       */
   double SolverGenWLMGEX::getFixedSolution( const TypeID& type,
                                         const SourceID& source ) const
      throw(InvalidRequest)
   {
