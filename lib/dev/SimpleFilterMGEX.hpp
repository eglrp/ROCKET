...
/**
 * @file SimpleFilterMGEX.hpp
 * This class filters out satellites with observations grossly out of bounds.
 */

#ifndef GPSTK_SIMPLEFILTERMGEX_HPP
#define GPSTK_SIMPLEFILTERMGEX_HPP
...
namespace gpstk
{
       ...
       * epoch.
       *
       * The "SimpleFilterMGEX" object will visit every satellite in the GNSS data
       ...
       *
       */
   class SimpleFilterMGEX : public ProcessingClass
   {
   public:

         /// Default constructor. By default, filter C1.
//      SimpleFilterMGEX() : minLimit(15000000.0), maxLimit(30000000.0)
//      { setFilteredType(TypeID::C1); };
		...
		SimpleFilterMGEX();
          ...
          */
//      SimpleFilterMGEX( const TypeID& type,
//                    const double& min,
//                    const double& max )
...
//      { setFilteredType(type); };
          ...
          * @param max       Maximum limit (in meters).
          */
//      SimpleFilterMGEX( const TypeIDSet& typeSet,
//                    const double& min,
//                    const double& max )
...
//      { };
      ...
      { Process(gData.body); return gData; };
          ...
          * @param min       Minimum limit (in meters).
          */
      virtual SimpleFilterMGEX& setMinLimit(const double& min)
      ...
      { return minLimit; };
          ...
          * @param max       Maximum limit (in meters).
          */
      virtual SimpleFilterMGEX& setMaxLimit(const double& max)
      ...
      { return maxLimit; };
          ...
          */
      virtual SimpleFilterMGEX& addFilteredType(const TypeID& type)
      { filterTypeSet.insert(type); return (*this); };


         /** Method to add a TypeID to be filtered.
			 * @param type		  Sat system
          * @param type      Extra TypeID to be filtered.
          */
      virtual SimpleFilterMGEX& addFilteredType(
									SatID::SatelliteSystem sys,
									const TypeID& type)
      { 
			( sysFilterTypeSet[ sys ] ).insert( type );
			return (*this);
		};
          ...
          * @param type      TypeID to be filtered.
          */
      virtual SimpleFilterMGEX& setFilteredType(const TypeID& type)
      { filterTypeSet.clear(); filterTypeSet.insert(type); return (*this); };
          ...
          * @param type      TypeID to be filtered.
          */
      virtual SimpleFilterMGEX& setFilteredType(
										SatID::SatelliteSystem sys,																		 const TypeID& type)
      {  
			( sysFilterTypeSet[ sys ] ).clear();
          ...
          * @param typeSet       Set of TypeID's to be filtered.
          */
      virtual SimpleFilterMGEX& setFilteredType(const TypeIDSet& typeSet)
...
//      { return ( (value>=minLimit) && (value<=maxLimit) );};
      ...
      TypeIDSet filterTypeSet;
			...
			/// System <---> TypeIDSet
		std::map< SatID::SatelliteSystem, TypeIDSet > sysFilterTypeSet;

         /// Minimum value allowed for input data (in meters).
      double minLimit;

			/// System <---> Minium limit
		std::map< SatID::SatelliteSystem, double > sysMinLimit;

         /// Maximum value allowed for input data (in meters).
      double maxLimit;

			/// System <---> Maximun limit
		std::map< SatID::SatelliteSystem, double > sysMaxLimit;


   }; // End of class 'SimpleFilterMGEX'
