...
namespace gpstk
{
    ...
    void ROCKPressure::doCompute(CommonTime utc, EarthBody& rb, Spacecraft& sc)
    {
        ...
        // compute the B-angle between the sun and the SV-BODY +Z axis
        // for ROCK4, also same as sun-satellite-earth angle
        double anga = dot(svec,rvec);
        ...
        {
            d0 = 4.54e-5 / mass;
            xt = ( -4.55*std::sin(angb) +
                    0.08*std::sin(ang2b+0.9) -
                    0.06*std::cos(ang4b+0.08) +
                    0.08 )*1e-5/mass;
            zt = ( -4.54*std::cos(angb) +
                    0.20*std::sin(ang2b-0.3) -
                    0.03*std::sin(ang4b) )*1e-5/mass;
        }
        else if(block == "II"
             || block == "IIA")
        {
            d0 = 8.695e-5 / mass;
            xt = ( -8.96*std::sin(angb) +
                    0.16*std::sin(ang3b) +
                    0.10*std::sin(ang5b) -
                    0.07*std::sin(ang7b) )*1e-5/mass;
        ...
        {
            d0 = 11.15e-5 / mass;
            xt = ( -11.0*std::sin(angb) +
                     0.2*std::sin(ang3b) -
                     0.2*std::sin(ang5b) )*1e-5 / mass;
            zt = ( -11.3*std::cos(angb) -
                     0.1*std::cos(ang3b) -
                     0.2*std::cos(ang5b) )*1e-5 / mass;
        }
        else if(block == "IIF")
        {
            d0 = 16.7e-5 / mass;
        ...
        // last, get the partials of acceleration wrt scale factors of ROCK SRP model
        for(int i=0; i<3; ++i)
        {
            da_dSRP(i,0) = lambda * xt * xvec(i) * distfct;
            da_dSRP(i,1) = d0 * yvec(i) * distfct;
