...
private:
   ...
   ConfDataReader confReader;
      ...
      // Map to store the postion and variance for sources
   std::map<SourceID, double> sourcePosMap;
   std::map<SourceID, double> sourcePosVarMap;

      // Map to store the postion and variance for satellites 
   std::map<SatID, double> satPosMap;
   std::map<SatID, double> satPosVarMap;

      // Map to store the velocity and variance for satellites 
   std::map<SatID, double> satVelMap;
   std::map<SatID, double> satVelVarMap;
...
}; // End of 'rocket' class declaration



   // Let's implement constructor details
rocket::rocket(char* arg0)
   :
   BasicFramework(  arg0,
...
"process such data to determine the UPDs.\n\n" ),
   ...
   confFile( CommandOption::stdType,
             'c',
             "conffile",
   " [-c|--conffile]    Name of configuration file ('rocket.conf' by default).",
             false )
{
...
void rocket::spinUp()
{
   ...
   else
   {
      ...
      try
      {
         ...
         confReader.open( "rocket.conf" );
      }
      catch(...)
      {

         cerr << "Problem opening default configuration file 'rocket.conf'"
              << endl;
...
/////// Starting processing for ALL stations ////
   ...
   while ( (station = confReader.getEachSection()) != "" )
   {
      ...
      network.addRinexObsFile( confReader("rinexObsFile", station) );
  
   }  // End of 'try-catch' block
  
  
      // Add class for all stations
   SimpleFilter
   MWCSDetector
   ComputeWindUp
   ComputeSatPCenter
   ...
   SourceID refSource = network.sourceIDOfRinexObsFile( "NetworkDemo/acor1480.08o");
   ...
   while( network.readEpochData(gdsMap) )
   {
      try
      {
         if(firstTime)
         {
            CommonTime epoch(gdsMap.first);
            satOrbit = BroadcastOrbit.getOrbit(epoch);
         }

         solverUpdWL.TimeUpdate(gdsMap);
         solverUpdNL.TimeUpdate(gdsMap);

            // Get the predicted orbit and other parameters
         stateMap = solverUpdNL.getSatState(epoch);

            // Set the orbit for serveral classes
         rangeModel.setSatOrbit(stateMap);
  
            // Data preprocessing
         gdsMap >> pList;
   
         solverUpdWL.MeasUpdate();
         solverUpdNL.MeasUpdate();
   ...
   int windowSize( decimateInterval/sampleInterval );
      ...
      // Declare solver objects to window-smooth the MWubbena combinations
      // Warning: 
   ...
   //$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$
   //
   //  Define the MW/LC equations for orbit/clk/upd estimation
   //
   //$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$
  ...
  /////////////////////////
   ...
   while( !gdsMap.empty() )
   {
      ...
      CommonTime workEpoch( (*gds.begin()).first );
         ...
         // Now, Let's first do the time update !!!!!!!!
         //
         // After 'TimeUpdate', the position and velocity for
         // the source and satellite will be inserted in to 
         // the 'gdsMap', which will be used in the following
         // data preprocessing classes, such as 'RangeModel',
         // 'ComputeWindUp', 'CorrectObservables', etc.
      solverUpdWL.TimeUpdate(gds);
      solverUpdNL.TimeUpdate(gds);

         // Now, Let's run the preprocessing for the measurements
      gds >> pList;
...
start=clock();
      ...
      solverUpdWL.MeasUpdate(gds);
      solverUpdNL.MeasUpdate(gds);
...
totaltime=(double)(finish-start)/CLOCKS_PER_SEC;
   ...
   SP3EphList.clear();
      ...
      // The rest of the processing will be in method 'rocket::shutDown()'
   return;

}  // End of 'rocket::process()'


   // Method to print model values
void rocket::printModel( ofstream& modelfile,
                         const gnssRinex& gData,
                         int   precision )
{
...
}  // End of method 'rocket::printModel()'
...
void rocket::getBiases( const CommonTime& workEpoch,
                        const SolverGenWL& solverUpdWL,
                        const SolverGenNL& solverUpdNL)
{
      // Mulitmap used to store the clock data line 
...
}  // End of method 'rocket::getBiasesWL()'
...
}  // End of 'rocket::shutDown()'


   // Method to print clock data 
void rocket::printHeader( ofstream& outFile)
{
   ...
   outFile<< headerRecord << endl;
      ...
      // "SYS / PCVS APPLIED"
   headerRecord  = string("G") + string(1,' ');
   headerRecord += leftJustify("rocket",17) + string(1, ' ');
   ...
   // Method to print clock data 
void rocket::printBiasData( ofstream& updFile)
{
      // Time tolerance  
...
}  // End of method 'rocket::printBiasData()'


   // Method to print clock data 
void rocket::printClockData( ofstream& clkFile)
{
      // Time tolerance  
   double tolerance(0.1);
...
}  // End of method 'rocket::printClockData()'


   // Method to print clock data 
void rocket::printUpdFixedData( ofstream& updFile)
{
      // Time tolerance  
   double tolerance(0.1);
   ...
   // Method to print clock data 
void rocket::printClockFixedData( ofstream& clkFile)
{
      // Time tolerance  
   double tolerance(0.1);
...
}  // End of method 'rocket::printClockFixedData()'
