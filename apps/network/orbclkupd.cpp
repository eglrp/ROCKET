...
//  Revision
//
//  2015/04/06  new software for real-time orbit, clock and upd estimation for
//              gnss satellite.
//
//============================================================================
...
class oces : public BasicFramework
{
public:
...
private:
   ...
   void getBiases( const CommonTime& workEpoch,
                   const SolverUPDWL& solverUPDWL,
                   const SolverUPDNL& solverUPDNL);
...
"process such data to determine the UPDs.\n\n" ),
   ...
   confFile( CommandOption::stdType,
             'c',
             "conffile",
   " [-c|--conffile]    Name of configuration file ('oces.conf' by default).",
             false )
{
...
void oces::spinUp()
{
   ...
   else
   {
      ...
      try
      {
         ...
         confReader.open( "oces.conf" );
      }
      catch(...)
      {

         cerr << "Problem opening default configuration file 'oces.conf'"
              << endl;
...
void oces::preprocessing()
{
      ...
      while(rin >> gRin)
      {
         ...
         try
         {
            ...
            while(!gdsMap.empty())
            {
               ...
               solver.Predict(gData);

                  // Get satellite sate/covariance from filter 
               SatDataMap sData = solver.getPredictedData();

                  // Now, Let's set the satellite state for the class
                  // that need satellite orbits.
                  // RangeModel, EclipseDetector, CorrectObservables,... 
               rangeModel.setSatData(sData);
               eclipseDetector.setSatData(sData);
               corr.setSatData(sData);

                  // Now, Let's prepare the measurements for measure update
               gData >> rangeModel
                     >> eclipseDetector
                     >> corr
                     >> svPcenter
   ...
   //// End of preprocessing for ALL stations ////
      ...
      // The rest of the processing will be in method 'oces::shutDown()'
   return;
   ...
   // Method to print model values
void oces::printModel( ofstream& modelfile,
                         const gnssRinex& gData,
                         int   precision )
{
...
void oces::solve()
{
   //>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
   //
   // Step 0. Define classes to preprocessing the gnss data
   //
   //>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
   ...
   bool isNEU( confReader.getValueAsBoolean( "USENEU", station ) );
      ...
      // Get if we want results in ECEF or NEU reference system
   bool fixPos( confReader.getValueAsBoolean( "fixCoordinate", station ) );

      // Declare ppp solver
   SolverPPP pppSolver(isNEU, fixPos);
   ...
   //>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
   //
   // Step 1. Define the equations for orbit/clock/upd estimation
   //
   //>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
   ...
   RecBiasRandomWalkModel ocesMWModel;
   Variable ocesMW( TypeID::ocesMW, &ocesMWModel, true, false, 9.0e10 );
   ...
   Variable ambWL( TypeID::BWL, &ambWLModel, true, true );
      ...
      // MW Receiver bias variable
   SatBiasRandomWalkModel ocesSatMWModel;

   ocesSatMWModel.setQprime(3.0e-12);
   Variable ocesSatMW( TypeID::ocesSatMW, &ocesSatMWModel, false, true );
   ...
   //orbitModel.setOrbit(&orbitModel);
   //
   //Variable orbit(&orbitModel);
   //Varialbe velocity(&orbitModel);
   //Varialbe forcepara(&orbitModel);
   ...
   equMWRef.addVariable(ocesMW, true); // coefficient (1.0)
   equMWRef.addVariable(ocesSatMW, true, -1.0); // coefficient(-1.0)
   ...
   equMWMaster.addVariable(ocesSatMW, true, -1.0); // coefficient(-1.0)
   ...
   RecBiasRandomWalkModel ocesLCModel;
   Variable ocesLC( TypeID::ocesLC, &ocesLCModel, true, false, 9.0e10 );
   ...
   SatBiasRandomWalkModel ocesSatLCModel;
   ocesSatLCModel.setQprime(3.0e-1);
   Variable ocesSatLC( TypeID::ocesSatLC, &ocesSatLCModel, false, true );
   ...
   equBLCRef.addVariable(ocesLC, true);  // coefficient = 1.0
   equBLCRef.addVariable(ocesSatLC, true, -1.0); // coefficient(-1.0)
   ...
   equBLCMaster.addVariable(ocesSatLC, true, -1.0); 
   ...
   ////> Define the equation system
      ...
      // Setup equation system
   EquationSystem system2;
   system2.addEquation(equBLCRef);
   system2.addEquation(equBLCMaster);

      // Forward-backward solver
   SolverUPDNL solverUPDNL(system2);

   ////> Define the constraint systems.
   
      // Setup constraint equation system
   AmbiguityDatum ambConstrNL;
   ambConstrNL.setAmbType(ambNL);

      // Feed the 'solverGen' with 'constrSystem'
   solverUPDNL.setAmbiguityDatum(ambConstrNL);
   ...
   //>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
   //
   // Step 2. Get the initial satellite orbit/clock/erp values from broadcast
   // ephemeris
   //
   //>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
   //
   ...
   //
   // solverGenAR.setInitialState();
   // solverGenAR.setInitialCovariance();
   //

   //>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
   //
   // Step 3. Now, Let's estimate the satellite orbit/clock/upd in real-time
   //
   //>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
...
//    
//    SatIDSet visibleSatSet = gdsMap.getCurrentSatIDSet();
//    for(SatIDSet::iterator itSat=visibleSatSet)
//    {
//       Variable varOrbit(orbit);
//       Varialbe varVel(velocity)
//       Varialbe vardy(dy);
//       varOrbit.setSatellite();
//       velOrbit.setSatellite();
//       dyOrbit.setSatellite();
//
//       initOrbit= solverGenAR.getPredOrbit();
//       initVel = solverGenAR.getPredVel();
//       initDy = solverGenAR.getPredDy();
//
//       orbitModel.setInitialState((*itSat),
//                                  currState);
//    }
//
//    solverGenAR.Prepare();
//
//    solverGenAR.Predict(gds);
//    include: TimeUpdate 
//
//    predictState=solverGenAR.getPredictState();
//
//    eclipseModel.setEphemeris(predictState);
//    computeSatPCenter.setEphemeris(predictState);
//    corr.setEphemeris(preditState);
//    rangeModel.setEphemeris(predictState);
//
//    gnssDataMap gds2;
//    for(gnssDataMap::iterator it=gdsMap = )
//    {
//       gRin = (*it).getGnssRinex();
//
            // Store current epoch
...
//
//          // Insert new values into 'gdsMap'
//       gds.insert(gRin);
//
//    }

         // Compute the widelane biases by fixing widelane ambiguities
//    solverGenAR.Correct(gds);
//    include: AmbiguityConstr, measUpdate, ambiguityFixing
...
void oces::getBiases( const CommonTime& workEpoch,
                        const SolverUPDWL& solverUPDWL,
                        const SolverUPDNL& solverUPDNL)
{
      // Mulitmap used to store the clock data line 
       ...
       ++itSource)
   {
      ...
      try
      {
         ocesLC = solverUPDNL.getSolution(TypeID::ocesLC, *itSource);
         ocesMW = solverUPDWL.getSolution(TypeID::ocesMW, *itSource);
      }
      ...
      solutionRecord += rightJustify(asString(ocesLC, 3), 8); // instrument bias LC
      solutionRecord += string(1,' ');
      solutionRecord += rightJustify(asString(ocesMW, 3), 8); // instrument bias MW
      solutionRecord += string(1,' ');
       ...
       ++itSat)
   {
      ...
      try
      {
         ocesSatLC = solverUPDNL.getSolution(TypeID::ocesSatLC, *itSat);
         ocesSatMW = solverUPDWL.getSolution(TypeID::ocesSatMW, *itSat);
      }
      catch(InvalidRequest& e)
      {
         continue;
      ...
      solutionRecord += rightJustify(asString(2),2); // clock data number
      solutionRecord += string(3,' '); // parse space
      solutionRecord += rightJustify(asString(ocesSatLC, 3), 8); // instrument bias LC
      solutionRecord += string(1,' ');
      solutionRecord += rightJustify(asString(ocesSatMW, 3), 8); // instrument bias MW
      solutionRecord += string(1,' ');
...
}  // End of method 'oces::getBiasesWL()'
...
}  // End of 'oces::shutDown()'


   // Method to print clock data 
void oces::printBiasHeader( ofstream& solutionFile)
{
   ...
   solutionFile<< headerRecord << endl;
      ...
      // "SYS / PCVS APPLIED"
   headerRecord  = string("G") + string(1,' ');
   headerRecord += leftJustify("oces",17) + string(1, ' ');
   ...
   // Method to print clock data 
void oces::printBiasData( ofstream& solutionFile)
{
      // Time tolerance  
...
}  // End of method 'oces::printBiasData()'
